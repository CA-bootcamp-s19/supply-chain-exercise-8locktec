...
contract SupplyChain {
  ...
  */
   ...
   */
  modifier forSale(uint sku) { require (items[sku].state == State.ForSale &&  items[sku].seller != address(0)); _; }
  ...
  function buyItem(uint sku) public payable
    forSale(sku) paidEnough(msg.value) checkValue(sku)  {
  ...
  shipped(sku) verifyCaller(items[sku].buyer) {
